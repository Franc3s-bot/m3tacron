...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 p-6 md:p-8 pb-20 lg:pb-8">
        ...
        <div class="mb-6"><LocalFilterBar id="lists-local" label="List filters" activeCount={listLocalCount} chips={listLocalChips} onRemoveChip={(k)=>filters.removeChip(k)} onClear={clearListFilters}><div class="flex flex-col gap-4"><FactionFilter /><StatRangeFilter label="Stat ranges" /><div class="rounded-xl border border-white/5 bg-black/20 p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"><PilotFilter selectedFactions={filters.selectedFactions} /></div><div class="rounded-xl border border-white/5 bg-black/20 p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"><ShipChassisFilter selectedFactions={filters.selectedFactions} /></div></div></LocalFilterBar></div>
