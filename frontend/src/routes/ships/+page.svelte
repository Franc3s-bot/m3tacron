...
<div class="flex min-h-screen">
    ...
    <main class="flex-1 p-6 md:p-8 pb-20 lg:pb-8">
        ...
        <div class="mb-6"><LocalFilterBar id="ships-local" label="Ship filters" activeCount={shipsLocalCount} chips={shipsLocalChips} onRemoveChip={(k)=>filters.removeChip(k)} onClear={clearShipsFilters}><div class="grid grid-cols-1 lg:grid-cols-2 gap-4"><div class="flex flex-col gap-4 self-start"><FactionFilter /><StatRangeFilter label="Stat ranges" /></div><div class="rounded-xl border border-white/5 bg-black/20 p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] self-start h-fit"><ShipChassisFilter selectedFactions={filters.selectedFactions} showModeToggle={false} /></div></div></LocalFilterBar></div>
