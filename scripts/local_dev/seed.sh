...
#   LOCAL_DEV_SSH_KEY      path to ssh key   (default: ~/.agents/skills/m3tacron/ssh_key)
...
SSH_KEY="${LOCAL_DEV_SSH_KEY:-$HOME/.agents/skills/m3tacron/ssh_key}"
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@$SSH_HOST" \
  "docker exec $DB_CONTAINER pg_dump -U postgres -Fc -f $REMOTE_DUMP postgres"

echo "==> Copying dump to local-data/dumps/ ..."
mkdir -p "$DUMPS_DIR"
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no \
  "$SSH_USER@$SSH_HOST:$REMOTE_DUMP" "$DUMPS_DIR/dev_latest.dump"

ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$SSH_USER@$SSH_HOST" \
  "rm -f $REMOTE_DUMP" || true
